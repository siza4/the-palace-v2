...
export async function grantInitialStanding(memberId) {
  ...
  const { data: standing, error } = await supabase
    ...
    .insert({
      ...
      contribution_expires_at: expires.toISOString()
    })
    .select()
    .single();
  ...
  // standing_history records the change; activity_logs is the unified
  // institutional-memory stream everything else writes to. This call
  // was missing before — grantInitialStanding was the one Standing
  // change in the codebase that didn't appear in activity_logs.
  await logActivity({
    memberId,
    action: 'STANDING_GRANTED',
    description: 'Member Standing automatically granted on admission.',
    metadata: { standing_level: 'Member Standing', grant_method: 'automatic' }
  });
...
/**
 * List Standing advancement requests, optionally filtered by status.
 * Used by the Butler's Office review queue.
 */
export async function listAdvancementRequests(status) {
  let query = supabase
    .from('standing_advancement_requests')
    .select('*, members:member_id (royal_id), standing_levels:requested_standing_id (name)')
...
/**
 * Get a member's current Standing record.
 * Used by the advancement request route to validate the member's
 * present Standing before accepting a new advancement request.
 */
