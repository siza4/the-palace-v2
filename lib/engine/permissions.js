...
export async function canAccessChamber(memberId, chamberId) {
  try {
    // Check if member has chamber access
    ...
    // Check if member has view_chamber permission (fallback)
    return await hasPermission(memberId, 'view_chamber');
  } catch (error) {
    console.error('Error checking chamber access:', error);
    return false;
  }
}
